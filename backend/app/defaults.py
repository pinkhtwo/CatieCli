...
DEFAULT_TUTORIAL_CONTENT = """# 🐱 Catiecli 完整使用教程
...
> 💡 **关于 GCP 项目**：OAuth 授权时会**自动创建项目**，通常不需要手动操作。如果你想了解 GCP 项目相关知识，可以展开下方参考。

<details>
### 📖 参考：手动创建 Google Cloud 项目（可选）

**通常不需要手动创建**，OAuth 授权会自动处理。以下仅供了解：
...
**注意**：看到「创建虚拟机」「在BigQuery中运行查询」「部署应用」等选项时，**全部忽略**，只需要空项目即可。
...
<details>
### Q: AI 回复重复/增殖（复读机）怎么办？

**情况**：AI 开始不断重复之前说过的话，像复读机一样。

**解决方法**：

1. **回退重新生成** - 回退到 AI 还没开始乱说话的那一层，点击「重新生成 (Roll)」按钮
2. **总结隐藏** - 如果上下文被"污染"，在输入框输入 `总结` 指令，把出问题的消息隐藏掉
3. **打开防缓存** - 有些预设里有"防缓存/随机数"选项，可以试着打开
4. **调整参数** - 降低 Temperature（0.7-0.9）、提高 Repetition Penalty（1.1-1.3）
...
<details>
### Q: Gemini 返回空消息或内容被拦截？

**情况**：看到 `PROHIBITED_CONTENT` 或回复为空。
